...
from preprocess_dataset.mel import wav2mel
...
cross_sample_dir = '/ceph/home/hujk17/Tuned-GE2E-EarSpeech/samples_Cross'



enc_model_fpath = None # GE2E 的模型路径写死在 Extractor 中
...
def main():
    ...
    for i, wav_name in enumerate(wav_list):
        for j, text in enumerate(input_text):
            ...
            print(GE2E_abs_path)
            # return
            embed_utterance((wav_path, GE2E_abs_path), encoder_model_fpath=Path('encoder/saved_models/pretrained.pt'))
            ...
            os.makedirs('log_FaPig_GE2E_syn_wavs_Cross', exist_ok=True)
            _wav_pre = mel2wav(mel, wav_name_path=os.path.join('log_FaPig_GE2E_syn_wavs_Cross', 'spk_' + str(i) + '_' + str(j) + '_pre.wav'))
            _wav_target = mel2wav(mel_reference, wav_name_path=os.path.join('log_FaPig_GE2E_syn_wavs_Cross', 'spk_' + str(i) + '_' + str(j) + '_reference.wav'))
            # break
            # return
